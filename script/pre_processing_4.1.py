...
def mergeLeaveOneOut2(df, dfv, vn):
    _key_codes = df[vn].astype('category').values.codes
    vn_yexp = 'exp2_' + vn
    grp1 = df[vn_yexp].groupby(_key_codes)
    _mean1 = grp1.aggregate(np.mean)
    _mean = _mean1[dfv[vn].astype('category').values.codes].values
...
for vn in vns:
    all_data['exp2_'+vn] = exp2_dict[vn]
os.getcwd()


# In[36]:

joblib.dump(all_data, "../processed/all_data_p3")
