...
all_data_p2 = joblib.load('../processed/all_data_p2')
instance_id = joblib.load('../processed/instance_id')

cvrt_value = all_data_p2['label']
day_values = all_data_p2['click_day']


xgb_param = {'max_depth': 7, 'eta': .15, 'objective':'binary:logistic', 'verbose':0,
         'subsample': 1.0, 'min_child_weight':5, 'gamma': 0,
...
xgb_feature = list(set(all_data_p2.columns) - set(["clickTime", "conversionTime", "label", "click_min"]))
...
for idx in [0, 1, 2, 3]:
    ...
    xt1 = all_data_p2.ix[filter1, xgb_feature]
    yt1 = cvrt_value[filter1]

    xv1 = all_data_p2.ix[filter_v1, xgb_feature]
...
print('-' * 30, logloss(predv_xgb, cvrt_value[filter_v1]))
...
pred_output = generate_pred_output(all_data_p2, xgb_param, xgb_feature, n_trees = 400, day_test=31)
